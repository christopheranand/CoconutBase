...
module Coconut.Graph.Utils.Dot
        (
          -- * Dot
          Dot           -- abstract
          -- * Nodes
...
-- 'showDot' renders a dot graph as a 'String'.
showDotUndirected :: Dot a -> String
showDotUndirected (Dot dm) = case dm 0 of
                    (elems,_,_) -> "graph G {\n" ++ unlines (map (showGraphElement " -- ") elems) ++ "\n}\n"

showGraphElement :: String -> GraphElement -> String
showGraphElement arr (GraphAttribute name val) = showAttr (name,val) ++ ";"
showGraphElement arr (GraphNode nid attrs)           = show nid ++ showAttrs attrs ++ ";"
showGraphElement arr (GraphEdge from to attrs) = show from ++ arr ++ show to ++  showAttrs attrs ++ ";"
showGraphElement arr (GraphEdge' from optF to optT attrs) = showName from optF ++ arr ++ showName to optT ++  showAttrs attrs ++ ";"
    where showName n Nothing = show n
          showName n (Just t) = show n ++ ":" ++ t
showGraphElement arr (Scope elems) = "{\n" ++ unlines (map (showGraphElement arr) elems) ++ "\n}"
showGraphElement arr (SubGraph nid elems) = "subgraph " ++ show nid ++ " {\n" ++ unlines (map (showGraphElement arr) elems) ++ "\n}"
